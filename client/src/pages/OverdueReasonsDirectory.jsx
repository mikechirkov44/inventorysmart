...
import { overdueReasonsAPI } from '../services/api';
...
function OverdueReasonsDirectory() {
  ...
  const fetchData = async () => {
    try {
      const res = await overdueReasonsAPI.getAll();
      setItems(res.data);
      setLoading(false);
    } catch {
      setLoading(false);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editing) {
        await overdueReasonsAPI.update(editing.id, form);
        toast.success('Обновлено');
      } else {
        await overdueReasonsAPI.create(form);
  ...
  const handleDelete = async (id) => {
    const confirmed = await confirm({ title: 'Удалить?', message: 'Причина просрочки будет удалена.', type: 'danger' });
    if (!confirmed) return;
    try {
      await overdueReasonsAPI.delete(id);
  ...
  return (
    <div className="directory-page">
      <PageHeader icon={Clock} title="Причины просрочки">
      ...
      {items.length === 0 && !search ? (
        <EmptyState
          icon={Clock}
          title="Причины просрочки ещё не добавлены"
          description="Создайте справочник причин для фиксации причин просроченных работ."
      ...
      {showForm && (
        <div className="complete-task-modal" onClick={() => setShowForm(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <h3>{editing ? 'Редактирование' : 'Новая причина просрочки'}</h3>
            <div className="form-group">
              <label>Название *</label>
              <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Например: Отсутствие запчастей" />
            </div>
...
export default OverdueReasonsDirectory;
